...
## İndir & Kur (kullanıcılar)

Son [GitHub Releases](../../releases) sayfasından platforma uygun artefaktı indir:

| Platform | İndirilecek dosya | Kurulum |
|----------|-------------------|---------|
| Linux (AppImage) | `TranslateApp-vX.Y.Z-x86_64.AppImage` | `chmod +x TranslateApp-*.AppImage && ./TranslateApp-*.AppImage` |
| Linux (tarball)  | `TranslateApp-vX.Y.Z-linux-x86_64.tar.gz` | Aç, içindeki `TranslateApp/TranslateApp` ikilisini çalıştır |
| Windows (installer) | `TranslateApp-vX.Y.Z-setup.exe` | Çift tıkla → kur (`%LOCALAPPDATA%\Programs\TranslateApp` altına kurulur, Start Menu kısayolu ekler) |
| Windows (zip) | `TranslateApp-vX.Y.Z-windows-x64.zip` | Aç, içindeki `TranslateApp.exe`'yi çalıştır |

Kullanıcı verisi (kelimeler veritabanı, ayarlar):
- **Linux:** `~/.local/share/TranslateApp/TranslateApp/`
- **Windows:** `%APPDATA%\TranslateApp\TranslateApp\`

> Linux'ta overlay'in tam çalışması için aşağıdaki **sistem paketleri** ve **`input` grubu üyeliği** hâlâ gerekir — AppImage bu sistem araçlarını bundle etmez. Yokken uygulama çalışır, ama Ctrl+C+C ve seçim algılama sessizce devre dışı kalır.
...
Kendi binary'ini üretmek için (release pipeline'ı taglı commit ile bunu otomatik yapar):

```bash
# Bağımlılıklar
pip install -r requirements.txt -r requirements-dev.txt

# Linux: AppImage + tar.gz üretir
bash scripts/build_linux.sh
# Build host'ta gereken sistem paketleri:
#   sudo apt install libxcb-cursor0 libxkbcommon-x11-0 libfuse2 fonts-dejavu-core
...
# Windows: ZIP + Inno Setup installer üretir (PowerShell'de)
.\scripts\build_windows.ps1
# Build host'ta gereken: choco install innosetup
```

Çıktılar repo kökünde oluşur (`TranslateApp-*.AppImage`, `*.tar.gz`, `*.zip`, `Output/*-setup.exe`).

### Otomatik release pipeline

`v0.1.0` gibi bir tag push edildiğinde [.github/workflows/build-release.yml](.github/workflows/build-release.yml) çalışır:
- `ubuntu-latest` + `windows-latest` matrix paralel build yapar.
- Her dört artefakt (`.AppImage`, `.tar.gz`, `.zip`, `-setup.exe`) GitHub Release'e otomatik eklenir.
